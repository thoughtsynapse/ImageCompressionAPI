...
http.createServer(function (req, res) {
  if (req.url == '/api') {
    ...
    form.parse(req, function (err, fields, files) {
      ...
      var newpath = '/Users/abdullah/Compressor/' + files.imageComp.name;
      fs.rename(oldpath, newpath, function (err) {
        ...
        if (files.imageComp.name.split('.').pop() === 'JPG' || files.imageComp.name.split('.').pop() === 'JPEG' || files.imageComp.name.split('.').pop() === 'jpg' || files.imageComp.name.split('.').pop() === 'jpeg') {
            ...
            const comImage = spawn( 'jpegoptim', ['-m90', '--strip-all', '/Users/abdullah/Compressor/' + files.imageComp.name]);
            ...
            // Compressing it
            const { spawn } = require( 'child_process' );
            const comImage = spawn( 'pngquant', ['--force', '--skip-if-larger', '--speed=1', '--strip', '--quality=60-90', '/Users/abdullah/Compressor/' + files.imageComp.name]);
            ...
            // Compressing it
            const { spawn } = require( 'child_process' );
            const comImage = spawn( 'jpegoptim', ['-m85', '--strip-all', '/Users/abdullah/Compressor/' + files.imageComp.name]);
